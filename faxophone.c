	...
	_cstruct b3_configuration)
{
	...
	if (intern) {
		...
		memcpy((char *) bc, "\x03\xE0\x90\xA3", sizeof(bc));
	} else {
	...
	memcpy((char *) llc, "\x02\x80\x90", sizeof(llc));

	if (cip == 0x04) {
		memcpy((char *) hlc, "\x02\x91\x81", sizeof(hlc));
	} else if (cip == 0x11) {
...
static struct capi_connection *capi_find_plci(int plci)
{
	...
	for (index = 0; index < CAPI_CONNECTIONS; index++) {
		if ((int)phsession->connection[index].plci == plci) {
...
static struct capi_connection *capi_find_ncci(int ncci)
{
	...
	for (index = 0; index < CAPI_CONNECTIONS; index++) {
		if ((int)phsession->connection[index].ncci == ncci) {
			return &phsession->connection[index];
		}
	}
